...
- `GET /error?word=...` — фронтенд (`quiz.html`) дёргает на неправильный клик; сервер апендит слово в `errors.txt`.
...
**Сохранение ошибок (`FileService.saveWrongWord`):**

`errors-filename` в `application.yaml` указывает на `./src/main/resources/errors.txt` — относительный путь от cwd при запуске. Это значит: при `./gradlew bootRun` из корня репозитория ошибки пишутся **прямо в исходники**, и `errors.txt` коммитится в git (см. недавние коммиты `Save quiz errors`, `More words`). Это сознательный workflow: квиз → ошибки в файл → ручной разбор → перенос проблемных слов обратно в `questions.txt`.
